...
export const getFoldersWithEmailCount = async () => {
 ...
 const specialFolders = specialFoldersOrder
  .map((name) => result.rows
  .find((folder) => folder.name === name))
  .filter(Boolean) as Folder[]

 const otherFolders = result.rows
  .filter((folder) => !specialFoldersOrder
  .includes(folder.name)) as Folder[]
...
export const getEmailsForFolder = async (
 folderName: string,
 search?: string
) => {
...
export const getEmailInFolder = async (
 folderName: string,
 emailId: string
) => {
 const originalFolderName = toTitleCase(decodeURIComponent(folderName))
